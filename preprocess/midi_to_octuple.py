...
def F(file_name):
    ...
    try:
        ...
        try:
            lock_write.acquire()
            out_path = f"lmd_matched_octuple/{file_name.split('/')[-1].replace('.mid', '.txt')}"
...
def sample():
    output_path = './lmd_Octuple'
    input_dir = './lmd_matched_flat'
    midi_paths = glob.glob(f'{input_dir}/*.mid')
    gen_dictionary(f'./{output_path}/dict.txt')
    ok_cnt = 0
    all_cnt = 0
    for midi_path in midi_paths:
        file_name = midi_path.replace(f'{input_dir}/', '')
        with Pool(pool_num) as p:
            output_file = f'./lmd_Octuple/{file_name}'
            result = list(p.imap_unordered(G, [midi_path]))
            print(f'{output_file}')
